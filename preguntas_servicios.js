const bancoPreguntas = {
  constitucion: {
    estructura: [
      {
        texto: "Desde el punto de vista formal, la Constitución Española de 1978 se considera:",
        opciones: ["Una ley orgánica que regula derechos fundamentales", "Una norma de carácter simbólico sin valor jurídico", "La norma suprema del ordenamiento jurídico español", "Un conjunto de normas dispersas con valor inferior a las leyes"],
        correcta: 2,
        explicacion: "La norma suprema del ordenamiento jurídico español."
      },
     {
        texto: "¿Cuál de los siguientes Títulos de la Constitución regula los derechos y deberes fundamentales?",
        opciones: ["Título Preliminar", "Título I", "Título II", "Título IV"],
        correcta: 1,
        explicacion: "Título I. De los derechos y deberes fundamentales."
      },
  {
    texto: "¿Qué parte de la Constitución Española comprende el Título Preliminar y el Título I?",
    opciones: ["Parte ejecutiva", "Parte legislativa", "Parte orgánica", "Parte dogmática"],
    correcta: 3,
    explicacion: "La parte dogmática comprende el Título Preliminar y el Título I."
  },
  {
    texto: "¿Qué recoge el Título Preliminar de la Constitución Española de 1978?",
    opciones: ["La organización territorial del Estado", "Los principios y valores del Estado", "El procedimiento de reforma constitucional", "La estructura de los tribunales"],
    correcta: 1,
    explicacion: "El Título Preliminar contiene los principios y valores fundamentales del Estado."
  },
      {
    texto: "¿Qué parte de la Constitución comprende los Títulos II a X?",
    opciones: ["Parte ejecutiva", "Parte orgánica", "Parte dogmática", "Parte institucional"],
    correcta: 1,
    explicacion: "La parte orgánica abarca los Títulos II a X."
  },
       {
    texto: "¿Qué establece la parte orgánica de la Constitución?",
    opciones: ["La inviolabilidad del Rey", "La estructura del poder judicial", "La división de poderes y organización territorial", "Los derechos laborales"],
    correcta: 2,
    explicacion: "La parte orgánica regula la división de poderes y la organización territorial del Estado."
  },
      {
    texto: "¿Qué título regula el Gobierno y la Administración?",
    opciones: ["Título II", "Título IV", "Título V", "Título VI"],
    correcta: 1,
    explicacion: "El Título IV regula el Gobierno y la Administración."
  },
      {
    texto: "¿Cuál es el contenido principal del Título V?",
    opciones: ["Relaciones entre Gobierno y Cortes Generales", "Derechos sociales", "Organización económica", "Tribunal Constitucional"],
    correcta: 0,
    explicacion: "El Título V trata sobre la relación Gobierno-Cortes."
  },
      {
    texto: "¿Qué regula el Título VI de la Constitución?",
    opciones: ["Derechos fundamentales", "El Poder Judicial", "La Corona", "La Hacienda Pública"],
    correcta: 1,
    explicacion: "El Título VI regula el Poder Judicial."
  },
    ],
    titulo1: [
      {
        texto: "¿Qué Título de la Constitución regula los derechos fundamentales?",
        opciones: ["Título Preliminar", "Título I", "Título II", "Título IV"],
        correcta: 1,
        explicacion: "El Título I (artículos 10 a 55) está dedicado a los derechos y deberes fundamentales."
      },
     {
    texto: "¿Cuál es la forma política del Estado según el artículo 1 de la Constitución?",
    opciones: ["República presidencialista", "Monarquía parlamentaria", "Estado federal", "Confederación parlamentaria"],
    correcta: 1,
    explicacion: "El artículo 1 establece que España es una Monarquía parlamentaria."
  },
    ],
    titulo2: [
      {
        texto: "¿Qué artículo regula la figura del Rey?",
        opciones: ["Artículo 56", "Artículo 66", "Artículo 97", "Artículo 123"],
        correcta: 0,
        explicacion: "El Artículo 56 establece que el Rey es el Jefe del Estado."
      },
      {
    texto: "¿Qué institución se considera símbolo de la unidad y permanencia del Estado?",
    opciones: ["El Congreso", "El Senado", "El Rey", "El Tribunal Constitucional"],
    correcta: 2,
    explicacion: "El Rey es símbolo de la unidad y permanencia del Estado."
  },
  {
    texto: "¿Cuál de las siguientes funciones NO corresponde al Rey?",
    opciones: ["Representar a España internacionalmente", "Ejercer funciones asignadas por la Constitución", "Sancionar leyes por sí mismo", "Arbitrar el funcionamiento de las instituciones"],
    correcta: 2,
    explicacion: "El Rey no puede sancionar leyes por sí mismo, necesita refrendo."
  },
  {
    texto: "¿Qué artículos de la Constitución regulan la inviolabilidad del Rey?",
    opciones: ["Del 56 al 58", "El artículo 56", "El artículo 1", "El artículo 66"],
    correcta: 1,
    explicacion: "El artículo 56 recoge la inviolabilidad del Rey."
  },
    ],
     titulo3: [
    {
    texto: "¿Qué órganos forman las Cortes Generales?",
    opciones: ["Congreso y Consejo de Estado", "Senado y Tribunal Supremo", "Congreso y Senado", "Gobierno y Congreso"],
    correcta: 2,
    explicacion: "Las Cortes Generales están formadas por el Congreso y el Senado."
  },
  {
    texto: "¿Cuál es una función de las Cortes Generales según el artículo 66?",
    opciones: ["Ejecutar leyes", "Interpretar la Constitución", "Controlar al Gobierno", "Nombrar al Rey"],
    correcta: 2,
    explicacion: "Las Cortes Generales controlan al Gobierno, entre otras funciones."
  },
],
     titulo4: [
       {
    texto: "¿Quién compone el Gobierno según el artículo 98?",
    opciones: ["El Rey y los Ministros", "El Presidente y el Congreso", "El Presidente, Vicepresidentes, Ministros y otros miembros que establezca la ley", "El Senado"],
    correcta: 2,
    explicacion: "El artículo 98 define la composición del Gobierno."
  },
       {
    texto: "¿Qué principios rigen la actuación de la Administración Pública según el artículo 103?",
    opciones: ["Jerarquía militar y secreto oficial", "Eficiencia, objetividad y legalidad", "Federalismo y pluralismo", "Privacidad y descentralización fiscal"],
    correcta: 1,
    explicacion: "El artículo 103 menciona eficacia, jerarquía, descentralización, etc."
  },
       ],
     titulo5: [
       {
    texto: "¿Ante quién responde el Gobierno en su gestión política según el artículo 108?",
    opciones: ["El Rey", "El Senado", "El Congreso de los Diputados", "El Consejo de Ministros"],
    correcta: 2,
    explicacion: "El artículo 108 indica que el Gobierno responde ante el Congreso."
  },
       ],
     titulo6: [
      {
    texto: "¿Qué caracteriza a los jueces y magistrados según el artículo 117?",
    opciones: ["Electos por sufragio", "Nombrados por el Congreso", "Independientes e inamovibles", "Designados por el Rey"],
    correcta: 2,
    explicacion: "El artículo 117 garantiza independencia, inamovilidad, etc."
  },
       ],
     titulo7: [
     {
    texto: "¿Qué reconoce el Título VII sobre la economía?",
    opciones: ["La libertad de mercado y la planificación pública", "El monopolio estatal", "La economía socialista", "El sistema comunal de bienes"],
    correcta: 0,
    explicacion: "El Título VII reconoce libertad de empresa y planificación económica pública."
  },
       ],
     titulo8: [
     {
    texto: "¿Qué entidades territoriales regula el Título VIII de la Constitución Española?",
    opciones: ["Ministerios, provincias y regiones", "Municipios, provincias y comunidades autónomas", "Diputaciones, concejos y regiones", "Gobiernos locales y provinciales"],
    correcta: 1,
    explicacion: "El Título VIII regula la organización territorial en municipios, provincias y comunidades autónomas."
  },
        {
    texto: "Según el artículo 137 de la Constitución, las entidades territoriales gozan de:",
    opciones: ["Poder legislativo independiente", "Autonomía para la gestión de sus respectivos intereses", "Dependencia directa del Gobierno central", "Autoridad constitucional exclusiva"],
    correcta: 1,
    explicacion: "El artículo 137 reconoce la autonomía de municipios, provincias y comunidades autónomas para gestionar sus intereses."
  },
        {
    texto: "¿Qué establece el Título VIII además de la organización territorial?",
    opciones: ["El sistema electoral de las comunidades", "La estructura del poder judicial", "La distribución de competencias entre el Estado y las comunidades autónomas", "La financiación local"],
    correcta: 2,
    explicacion: "También establece la distribución de competencias entre el Estado y las Comunidades Autónomas."
  },
       ],
     titulo9: [
     {
    texto: "¿Qué órgano regula el Título IX de la Constitución?",
    opciones: ["El Consejo de Estado", "El Tribunal Supremo", "El Tribunal Constitucional", "El Tribunal de Cuentas"],
    correcta: 2,
    explicacion: "El Título IX regula el Tribunal Constitucional."
  },
        {
    texto: "¿Qué función principal tiene el Tribunal Constitucional según la Constitución?",
    opciones: ["Redactar leyes orgánicas", "Designar magistrados", "Controlar a los partidos políticos", "Garantizar el cumplimiento de los principios y normas constitucionales"],
    correcta: 3,
    explicacion: "El Tribunal Constitucional garantiza que no se vulneren los principios y normas constitucionales."
  },
       ],
     titulo10: [
    {
    texto: "¿Qué regula el Título X de la Constitución Española de 1978?",
    opciones: ["El sistema electoral", "La estructura del Senado", "La reforma de la Constitución", "Las competencias municipales"],
    correcta: 2,
    explicacion: "El Título X regula el procedimiento para reformar la Constitución."
  },
        {
    texto: "¿Qué exige el procedimiento de reforma constitucional si afecta a determinados preceptos?",
    opciones: ["Aprobación por mayoría simple del Congreso", "Informe favorable del Rey", "Aprobación por referéndum popular", "Ratificación por el Tribunal Constitucional"],
    correcta: 2,
    explicacion: "En algunos casos, la reforma requiere aprobación por referéndum popular."
  },
         {
    texto: "¿Qué mayorías se requieren para la reforma constitucional ordinaria?",
    opciones: ["Mayoría absoluta en el Congreso", "Mayoría cualificada en ambas Cámaras", "Mayoría simple en el Senado", "Unanimidad del Consejo de Ministros"],
    correcta: 1,
    explicacion: "Se exige mayoría cualificada en el Congreso y el Senado."
  },
       {
    texto: "¿Cuántas veces ha sido modificada la Constitución Española de 1978 hasta la actualidad?",
    opciones: ["Dos", "Tres", "Cinco", "Ninguna"],
    correcta: 1,
    explicacion: "Se ha modificado en tres ocasiones: 1992 (art. 13.2), 2011 (art. 135) y 2024 (art. 49)."
  },
       ]
  },
  estatuto: {
    general: [
      {
        texto: "¿En qué año se aprobó el Estatuto de Autonomía de Murcia?",
        opciones: ["1978", "1982", "1991", "2005"],
        correcta: 1,
        explicacion: "Fue aprobado por Ley Orgánica 4/1982 de 9 de junio."
      },
      // Más preguntas...
    ]
  },
  prevencion_riesgos: {
    basicos: [
      {
        texto: "¿Qué color identifica las señales de prohibición?",
        opciones: ["Rojo", "Amarillo", "Azul", "Verde"],
        correcta: 0,
        explicacion: "Las señales de prohibición son redondas con fondo blanco y borde rojo."
      }
    ]
  }
};
